...
handleTransaction=async()=>{
 ...
 Alert.alert("Infelismente nós ainda não temos esse livro, tente pegar outro.");
...
checkStudentForIssue=async studentId=>{
  ...
  if(studentRef.docs.length==0){
    ...
    Alert.alert("Esse aluno nãq está mais na escola.")
  } else{
...
checkStudentForReturn=async(bookId,studentId)=>{
  ...
  transactionRef.docs.map(doc=>{
    ...
    if(lastBookTransaction.student_id === studentId){
      ...
      isStudentAvailable=false;
      Alert.alert("Me desculpe mais ão foi vc quem pegou essse livro, e a pessoa que pegou pode ainda estar lendo.")
      this.setState({bookId:"",studentId:""})
    }
  })
  return isStudentAvailable;
